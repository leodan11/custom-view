...
import com.github.leodan11.customview.layout.toggle.Toggle
import com.github.leodan11.customview.layout.toggle.ToggleView
import com.github.leodan11.customview.layout.toggle.Utils
...
class ToggleButtonLayout : CardView {
     ...
     */
    var onToggledListener: ((toggleButtonLayout: ToggleButtonLayout, toggle: Toggle, selected: Boolean) -> Unit)? =
        null
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        getContext().withStyledAttributes(
            attrs, R.styleable.ToggleButtonLayout,
            0, 0
        ) {

            if (hasValue(R.styleable.ToggleButtonLayout_multipleSelection)) {
                multipleSelection =
                    getBoolean(R.styleable.ToggleButtonLayout_multipleSelection, false)
            }
            if (hasValue(R.styleable.ToggleButtonLayout_allowDeselection)) {
                allowDeselection = getBoolean(R.styleable.ToggleButtonLayout_allowDeselection, true)
            }
            if (hasValue(R.styleable.ToggleButtonLayout_dividerColor)) {
                dividerColor = getColor(R.styleable.ToggleButtonLayout_dividerColor, Color.GRAY)
            }
            if (hasValue(R.styleable.ToggleButtonLayout_customLayout)) {
                layoutRes = getResourceId(R.styleable.ToggleButtonLayout_customLayout, 0)
            }
            if (hasValue(R.styleable.ToggleButtonLayout_toggleMode)) {
                val mode = getInt(R.styleable.ToggleButtonLayout_toggleMode, MODE_WRAP)
                this@ToggleButtonLayout.mode = mode
            }
            ...
            if (hasValue(R.styleable.ToggleButtonLayout_menu)) {
                inflateMenu(getResourceId(R.styleable.ToggleButtonLayout_menu, 0))
            }
        }
    }
    ...
    private fun adjustColorOfSelected() {
        ...
        selectedToggles.forEach {
            ...
            view.background = selectedColor.toDrawable()
        }
    }
