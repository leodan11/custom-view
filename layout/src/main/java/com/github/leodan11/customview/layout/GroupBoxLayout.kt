...
import com.github.leodan11.customview.layout.groupbox.CutoutDrawable
...
class GroupBoxLayout : RelativeLayout {
    ...
    fun setLabelStyleResource(@StyleRes resId: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            binding.label.setTextAppearance(resId)
        } else {
    ...
    fun setCornerRadius(radius: Int) {
        val appearanceModel =
            drawable.shapeAppearanceModel.toBuilder().setAllCornerSizes(radius.toFloat()).build()
