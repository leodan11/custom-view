    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        if (direction == ItemTouchHelper.RIGHT) {
            if (mSwipedView.shouldShowRTL())
                mListener.onSwipedLeft(viewHolder.getBindingAdapterPosition());
            else
                mListener.onSwipedRight(viewHolder.getBindingAdapterPosition());
        } else {
            if (mSwipedView.shouldShowRTL())
                mListener.onSwipedRight(viewHolder.getBindingAdapterPosition());
            else
    ...
    private void draw(@NonNull Canvas c, Context context, ChildToDraw toDraw) {
        ...
        if (icon != null) {
            ...
            c.drawText(toDraw.getText(), bg.getBounds().centerX(),
                    bg.getBounds().centerY() + (float) mSwipedView.getTextSize() / 2, paint);
        }
    }

    private int returnMovementFlags() {
        if (mSwipedView.getLeftIcon() == -1 &&
                mSwipedView.getLeftBg() == android.R.color.white &&
                mSwipedView.getLeftText().isEmpty() &&
                mSwipedView.getRightIcon() == -1 &&
                mSwipedView.getRightBg() == android.R.color.white &&
    ...
    private class ChildToDraw {
        ...
        private ChildToDraw invoke() {
            ...
            try {
                ...
                icon = null;
                Log.e(ChildToDraw.class.getSimpleName(), "Failed to load icon", e);
            }
