...
public class NumberPicker extends LinearLayout {
    ...
    @DrawableRes
    private int downIcon = R.drawable.ic_arrow_down_baseline;

    @DrawableRes
    private int upIcon = R.drawable.ic_arrow_up_baseline;

    @DrawableRes
    private int leftIcon = R.drawable.ic_arrow_left_baseline;

    @DrawableRes
    private int rightIcon = R.drawable.ic_arrow_right_baseline;
    ...
    @SuppressWarnings("unused")
    public void setListener(@NonNull NumberPickerListener listener) {
        mListener = listener;
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        downIcon = typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonIconDown, downIcon);
        upIcon = typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonIconUp, upIcon);
        leftIcon = typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonIconLeft, leftIcon);
        rightIcon = typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonIconRight, rightIcon);

        mMinValue = typedArray.getInt(R.styleable.NumberPicker_snp_minValue, res.getInteger(R.integer.default_minValue));
        mMaxValue = typedArray.getInt(R.styleable.NumberPicker_snp_maxValue, res.getInteger(R.integer.default_maxValue));

        mStepSize = typedArray.getInt(R.styleable.NumberPicker_snp_stepSize, res.getInteger(R.integer.default_stepSize));

        mUpdateIntervalMillis = typedArray.getInt(R.styleable.NumberPicker_snp_updateInterval, res.getInteger(R.integer.default_updateInterval));

        mOrientation = typedArray.getInt(R.styleable.NumberPicker_snp_orientation, LinearLayout.HORIZONTAL);

        mValue = typedArray.getInt(R.styleable.NumberPicker_snp_value, res.getInteger(R.integer.default_value));

        mValueTextSize = typedArray.getDimension(R.styleable.NumberPicker_snp_value_text_size, INVALID_RES);
        mValueTextColor = typedArray.getColor(R.styleable.NumberPicker_snp_value_text_color, 0);
        mValueTextAppearanceResId = typedArray.getResourceId(R.styleable.NumberPicker_snp_value_text_appearance, INVALID_RES);

        mScrollEnabled = typedArray.getBoolean(R.styleable.NumberPicker_snp_scrollEnabled, false);

        mButtonColorStateList = ContextCompat.getColorStateList(context, typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonBackgroundTintSelector, R.color.default_btn_tint_selector));

        mValueMarginStart = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_valueMarginStart, res.getDimension(R.dimen.default_value_margin_start));
        mValueMarginEnd = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_valueMarginStart, res.getDimension(R.dimen.default_value_margin_end));

        mButtonPaddingLeft = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_buttonPaddingLeft, res.getDimension(R.dimen.default_button_padding_left));
        mButtonPaddingRight = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_buttonPaddingRight, res.getDimension(R.dimen.default_button_padding_right));
        mButtonPaddingTop = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_buttonPaddingTop, res.getDimension(R.dimen.default_button_padding_top));
        mButtonPaddingBottom = (int) typedArray.getDimension(R.styleable.NumberPicker_snp_buttonPaddingBottom, res.getDimension(R.dimen.default_button_padding_bottom));

        TypedValue outValue = new TypedValue();
        res.getValue(R.dimen.default_button_scale_factor, outValue, true);
        float defaultValue = outValue.getFloat();
        mButtonTouchScaleFactor = typedArray.getFloat(R.styleable.NumberPicker_snp_buttonTouchScaleFactor, defaultValue);
    ...
    private void initButtonPlusView() {
        ...
        mPlusButton.setOnClickListener(v -> increment());

        mPlusButton.setOnLongClickListener(v -> {
            mAutoIncrement = true;
            mUpdateIntervalHandler.post(new RepeatRunnable());
            return false;
        });

        mPlusButton.setOnTouchListener((v, event) -> {
    ...
    private void initButtonMinusView() {
        ...
        mMinusButton.setOnClickListener(v -> decrement());

        mMinusButton.setOnLongClickListener(v -> {
            mAutoDecrement = true;
            mUpdateIntervalHandler.post(new RepeatRunnable());
            return false;
        });

        mMinusButton.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                scaleImageViewDrawable(mMinusButton, mButtonTouchScaleFactor);
            } else if (event.getAction() == MotionEvent.ACTION_UP) {
                if (mAutoDecrement) {
